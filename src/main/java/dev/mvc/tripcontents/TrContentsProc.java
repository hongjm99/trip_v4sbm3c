...
@Component("dev.mvc.tripcontents.TrContentsProc")
public class TrContentsProc implements TrContentsProcInter {
  @Autowired  // ContentsDAOInter interface를 구현한 클래스의 객체를 만들어 자동으로 할당해라.
  private TrContentsDAOInter contentsDAO;
  ...
  public ArrayList<TrContentsVO> list_all() {
    ArrayList<TrContentsVO> list = this.contentsDAO.list_all();
    return list;
  }

  @Override
  public ArrayList<TrContentsVO> list_by_tripno(int tripno) {
    ArrayList<TrContentsVO> list = this.contentsDAO.list_by_tripno(tripno);
    return list;
  }
  
  @Override
  public TrContentsVO read(int scontentsno) {
    TrContentsVO contentsVO = this.contentsDAO.read(scontentsno);
    return contentsVO;
  }
  ...
  public ArrayList<TrContentsVO> list_by_tripno_search(HashMap<String, Object> hashMap) {
    ArrayList<TrContentsVO> list = this.contentsDAO.list_by_tripno_search(hashMap);
    return list;
  }
  ...
  public ArrayList<TrContentsVO> list_by_tripno_search_paging(TrContentsVO contentsVO) {
    ...
    */
    int begin_of_page = (contentsVO.getNow_page() - 1) * TrContents.RECORD_PER_PAGE;
    ...
    // 2 페이지 = 10 + 10: 20
    // 3 페이지 = 20 + 10: 30
    int end_num = begin_of_page + TrContents.RECORD_PER_PAGE;   
    /*
    ...
    */
    ...
    ArrayList<TrContentsVO> list = this.contentsDAO.list_by_tripno_search_paging(contentsVO);
...
//  
//  <DIV id='paging'>
//    <span class='span_box_1'><A href='list_by_tripno.do?word=&now_page=1&tripno=4'>1</A></span>
//  <span class='span_box_2'>2</span>
//  <span class='span_box_1'><A href='list_by_tripno.do?word=&now_page=3&tripno=4'>3</A></span>
//    </DIV>
//  </DIV> 
//  <!-- 페이지 목록 출력 부분 종료 -->
    ...
    public String pagingBox(int tripno, int now_page, String word, String list_file, int search_count){
      ...
      // 전체 페이지 수 :(double)1/10 = 0.1 -> 1페이지, (double)12/10 -> 2 페이지
      int total_page = (int)(Math.ceil((double)search_count / TrContents.RECORD_PER_PAGE)); 
      // 전체 그룹  수: (double)1/10 -> 0.1 -> 1 그룹, (double)12/10 -> 1.2 그룹 -> 2 그룹
      int total_grp = (int)(Math.ceil((double)total_page / TrContents.PAGE_PER_BLOCK)); 
      // 현재 그룹 번호: (double)13/10 -> 1.3 -> 2 그룹
      int now_grp = (int)(Math.ceil((double)now_page / TrContents.PAGE_PER_BLOCK));  
      
      // 1 group: 1, 2, 3 ... 9, 10
      // 2 group: 11, 12 ... 19, 20
      // 3 group: 21, 22 ... 29, 30
      int start_page = ((now_grp - 1) * TrContents.PAGE_PER_BLOCK) + 1; // 특정 그룹의 시작  페이지  
      int end_page = (now_grp * TrContents.PAGE_PER_BLOCK);               // 특정 그룹의 마지막 페이지   
...
//      str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
      ...
      int _now_page = (now_grp - 1) * TrContents.PAGE_PER_BLOCK;  
      if (now_grp >= 2){ // 현재 그룹번호가 2이상이면 페이지수가 11페이 이상임으로 이전 그룹으로 갈수 있는 링크 생성 
        str.append("<span class='span_box_1'><A href='"+list_file+"?&word="+word+"&now_page="+_now_page+"&tripno="+tripno+"'>이전</A></span>"); 
      } 
      ...
      for(int i=start_page; i<=end_page; i++){ 
        ...
        if (now_page == i){ // 목록에 출력하는 페이지가 현재페이지와 같다면 CSS 강조(차별을 둠)
          ...
          str.append("<span class='span_box_1'><A href='"+list_file+"?word="+word+"&now_page="+i+"&tripno="+tripno+"'>"+i+"</A></span>");   
        } 
      } 
      ...
      _now_page = (now_grp * TrContents.PAGE_PER_BLOCK)+1; //  최대 페이지수 + 1 
      if (now_grp < total_grp){ 
        str.append("<span class='span_box_1'><A href='"+list_file+"?&word="+word+"&now_page="+_now_page+"&tripno="+tripno+"'>다음</A></span>"); 
      } 
      str.append("</DIV>"); 
    ...
    public int delete(int scontentsno) {
      int cnt = this.contentsDAO.delete(scontentsno);
      return cnt;
    }
    
    @Override
    public int count_by_tripno(int tripno) {
      int cnt = this.contentsDAO.count_by_tripno(tripno);
      return cnt;
    }
    
    @Override
    public int delete_by_tripno(int tripno) {
      int cnt = this.contentsDAO.delete_by_tripno(tripno);
      return cnt;
    }
}
