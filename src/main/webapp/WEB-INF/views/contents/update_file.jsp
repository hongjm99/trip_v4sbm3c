...
<c:import url="/menu/top.do" />
  <DIV class='title_line'> ${tripVO.name } > ${title } >파일 수정</DIV>
  ...
  <aside class="aside_right">
    ...
    <span class='menu_divide' >│</span>    
    <A href="./list_by_tripno.do?tripno=${tripno }&now_page=${param.now_page}&word=${param.word }">목록형</A>    
    <span class='menu_divide' >│</span>
    ...
    <form name='frm' id='frm' method='get' action='./list_by_tripno.do'>
      ...
      <c:if test="${param.word.length() > 0 }">
        <button type='button' class='btn btn-secondary btn-sm' 
                    onclick="location.href='./list_by_tripno.do?tripno=${tripVO.tripno}&word='" style="padding: 2px 8px 3px 8px; margin: 0px 0px 2px 0px;">검색 취소</button>  
      </c:if>    
    </form>
  </div>
